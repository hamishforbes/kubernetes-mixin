      ...
      },
      {
        ...
        rules: [
          ...
          },
          {
            ...
            expr: |||
              sum by(%(clusterGroupLabelsStr)s, instance, name, reason)(increase(aggregator_unavailable_apiservice_total{%(kubeApiserverSelector)s}[1m])) > 0
            ||| % $._config,
            'for': '10m',
            labels: {
              severity: 'warning',
          ...
          },
          {
            ...
            expr: |||
              sum by(%(clusterGroupLabelsStr)s) (rate(apiserver_request_terminations_total{%(kubeApiserverSelector)s}[10m])) / ( sum by(%(clusterGroupLabelsStr)s) (rate(apiserver_request_total{%(kubeApiserverSelector)s}[10m])) + sum by(%(clusterGroupLabelsStr)s) (rate(apiserver_request_terminations_total{%(kubeApiserverSelector)s}[10m])) ) > 0.20
            ||| % $._config,
            labels: {
              severity: 'warning',
            },
