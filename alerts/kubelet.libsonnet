    ...
    groups+: [
      {
        ...
        rules: [
          ...
          },
          {
            ...
            expr: |||
              (
                max by (%(clusterGroupLabelsStr)s, instance) (
              ...
              / on (%(clusterGroupLabelsStr)s, node) group_left()
              max by (%(clusterGroupLabelsStr)s, node) (
          ...
          },
          {
            ...
            expr: |||
              sum(changes(kube_node_status_condition{%(kubeStateMetricsSelector)s,status="true",condition="Ready"}[15m])) by (%(clusterGroupLabelsStr)s, node) > 2
              and on (%(clusterGroupLabelsStr)s, node)
              kube_node_spec_unschedulable{%(kubeStateMetricsSelector)s} == 0
            ||| % $._config,
            'for': '15m',
            labels: {
          ...
          },
          {
            ...
            expr: |||
              histogram_quantile(0.99,
                sum by (%(clusterGroupLabelsStr)s, instance, le) (
                  topk by (%(clusterGroupLabelsStr)s, instance, le, operation_type) (1,
                    rate(kubelet_pod_worker_duration_seconds_bucket{%(kubeletSelector)s}[5m])
                  )
                )
              )
              * on(%(clusterGroupLabelsStr)s, instance) group_left(node)
              topk by (%(clusterGroupLabelsStr)s, instance, node) (1,
                kubelet_node_name{%(kubeletSelector)s}
              )
              > 60
            ||| % $._config,
