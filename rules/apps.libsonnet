        ...
        rules: [
          {
            ...
            expr: |||
              sum by (%(clusterGroupLabelsStr)s, namespace, pod, container) (
                rate(container_cpu_usage_seconds_total{%(cadvisorSelector)s, image!=""}[5m])
              ) * on (%(clusterGroupLabelsStr)s, namespace, pod) group_left(node) topk by (%(clusterGroupLabelsStr)s, namespace, pod) (
                1, max by(%(clusterGroupLabelsStr)s, namespace, pod, node) (kube_pod_info{node!=""})
              )
            ||| % $._config,
          },
          {
        ...
        rules: [
          {
            ...
            expr: |||
              ...
              * on (%(clusterGroupLabelsStr)s, namespace, pod) group_left(node) topk by(%(clusterGroupLabelsStr)s, namespace, pod) (1,
                max by(%(clusterGroupLabelsStr)s, namespace, pod, node) (kube_pod_info{node!=""})
              )
            ||| % $._config,
          },
        ],
        ...
        rules: [
          {
            ...
            expr: |||
              kube_pod_container_resource_requests{resource="memory",%(kubeStateMetricsSelector)s}  * on (namespace, pod, %(clusterGroupLabelsStr)s)
              group_left() max by (namespace, pod, %(clusterGroupLabelsStr)s) (
          ...
          },
          {
            ...
            expr: |||
              sum by (namespace, %(clusterGroupLabelsStr)s) (
                  sum by (namespace, pod, %(clusterGroupLabelsStr)s) (
                      max by (namespace, pod, container, %(clusterGroupLabelsStr)s) (
                        kube_pod_container_resource_requests{resource="memory",%(kubeStateMetricsSelector)s}
                      ) * on(namespace, pod, %(clusterGroupLabelsStr)s) group_left() max by (namespace, pod, %(clusterGroupLabelsStr)s) (
                        kube_pod_status_phase{phase=~"Pending|Running"} == 1
                      )
                  )
              )
        ...
        rules: [
          {
            ...
            expr: |||
              kube_pod_container_resource_requests{resource="cpu",%(kubeStateMetricsSelector)s}  * on (namespace, pod, %(clusterGroupLabelsStr)s)
              group_left() max by (namespace, pod, %(clusterGroupLabelsStr)s) (
                (kube_pod_status_phase{phase=~"Pending|Running"} == 1)
              )
            ||| % $._config,
        ...
        rules: [
          {
            ...
            expr: |||
              kube_pod_container_resource_limits{resource="memory",%(kubeStateMetricsSelector)s}  * on (namespace, pod, %(clusterGroupLabelsStr)s)
              group_left() max by (namespace, pod, %(clusterGroupLabelsStr)s) (
                (kube_pod_status_phase{phase=~"Pending|Running"} == 1)
              )
            ||| % $._config,
        ...
        rules: [
          {
            ...
            expr: |||
              kube_pod_container_resource_limits{resource="cpu",%(kubeStateMetricsSelector)s}  * on (namespace, pod, %(clusterGroupLabelsStr)s)
              group_left() max by (namespace, pod, %(clusterGroupLabelsStr)s) (
                (kube_pod_status_phase{phase=~"Pending|Running"} == 1)
              )
            ||| % $._config,
      ...
      },
      {
        ...
        rules: [
          // workload aggregation for replicasets
          {
            ...
            expr: |||
              max by (%(clusterGroupLabelsStr)s, namespace, workload, pod) (
                label_replace(
                  label_replace(
                    kube_pod_owner{%(kubeStateMetricsSelector)s, owner_kind="ReplicaSet"},
                    "replicaset", "$1", "owner_name", "(.*)"
                  ) * on (%(clusterGroupLabelsStr)s, replicaset, namespace) group_left(owner_name) topk by(%(clusterGroupLabelsStr)s, replicaset, namespace) (
                    1, max by (%(clusterGroupLabelsStr)s, replicaset, namespace, owner_name) (
              ...
              max by (%(clusterGroupLabelsStr)s, namespace, workload, pod) (
                label_replace(
                  label_replace(
                    kube_pod_owner{%(kubeStateMetricsSelector)s, owner_kind="ReplicaSet"},
                    "replicaset", "$1", "owner_name", "(.*)"
                  ) * on(replicaset, namespace, %(clusterGroupLabelsStr)s) group_left(owner_name) topk by(%(clusterGroupLabelsStr)s, replicaset, namespace) (
                    1, max by (%(clusterGroupLabelsStr)s, replicaset, namespace, owner_name) (
          ...
          // backwards compatibility for jobs
          {
            record: 'namespace_workload_pod:kube_pod_owner:relabel',
            expr: |||
              group by (%(clusterGroupLabelsStr)s, namespace, workload, pod) (
                label_join(
                  group by (%(clusterGroupLabelsStr)s, namespace, job_name, pod, owner_name) (
                    label_join(
                      kube_pod_owner{%(kubeStateMetricsSelector)s, owner_kind="Job"}
                    , "job_name", "", "owner_name")
                  )
                  * on (%(clusterGroupLabelsStr)s, namespace, job_name) group_left()
                  group by (%(clusterGroupLabelsStr)s, namespace, job_name) (
          ...
          // workload aggregation for non-standard types (jobs, replicasets)
          {
            record: 'namespace_workload_pod:kube_pod_owner:relabel',
            expr: |||
              group by (%(clusterGroupLabelsStr)s, namespace, workload, workload_type, pod) (
                label_join(
                  label_join(
                    group by (%(clusterGroupLabelsStr)s, namespace, job_name, pod) (
                      label_join(
                        kube_pod_owner{%(kubeStateMetricsSelector)s, owner_kind="Job"}
                      , "job_name", "", "owner_name")
                    )
                    * on (%(clusterGroupLabelsStr)s, namespace, job_name) group_left(owner_kind, owner_name)
                    group by (%(clusterGroupLabelsStr)s, namespace, job_name, owner_kind, owner_name) (
                ...
                label_replace(
                  label_replace(
                    ...
                    * on(%(clusterGroupLabelsStr)s, namespace, replicaset) group_left(owner_kind, owner_name)
                    group by (%(clusterGroupLabelsStr)s, namespace, replicaset, owner_kind, owner_name) (
                      kube_replicaset_owner{%(kubeStateMetricsSelector)s, owner_kind!="Deployment", owner_kind!=""}
                    )
                  , "workload", "$1", "owner_name", "(.+)")
                  OR
                  label_replace(
                    group by (%(clusterGroupLabelsStr)s, namespace, pod, owner_name, owner_kind) (
